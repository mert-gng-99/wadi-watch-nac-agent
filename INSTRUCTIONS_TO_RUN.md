...
1. Enrolled, but outside the risk area, the first scenario on the left. Watch the agent pick
...
2. Run all seven alert cases runs all 7 scenarios back to back and shows the
...
```bash
docker build -t wadi-watch . && docker run -p 8000:8000 wadi-watch
```
