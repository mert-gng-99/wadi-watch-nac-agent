# Wadi Watch - Prototype Phase submission
...
**YOU.** Pick your Idea Phase submission for Wadi Watch from the dropdown.
...
```
6. Climate Resilience & Environmental Monitoring
```
...
1. The dashboard after Run all seven alert cases, showing the batch table and the cost comparison.
...
8 CAMARA APIs, 74.9% cheaper than calling every check.
...
```
https://github.com/mert-gng-99/wadi-watch-nac-agent
```
...
```
wadi-watch-nac-agent-source.zip
```
...
| --- | --- | --- |
| Parent Submission | Pick Wadi Watch from the dropdown | seconds |
...
| --- | --- |
| At least one CAMARA API on Nokia NaC | 8 API families: `location-verification`, `device-status`, `location-retrieval`, `congestion-insights`, `quality-on-demand`, `network-slice-device-attachment`, `geofencing-subscriptions`, `number-verification` |
...
| Solves a real problem | Climate Resilience & Environmental Monitoring. Stated limits are in README.md, so this is not pitched as doing more than it does. |
| Theme alignment | Theme 6 |
| Multiple CAMARA APIs (good-to-have) | 8 families orchestrated per case |
| Intelligent orchestration (good-to-have) | 1 to 6 calls depending on the case; 74.9% cheaper than calling everything |
